...
* **`browserSem`** — acquired only on escalation, _after_ `httpSem` is released
...
greenclaw uses Blink (via Chromium) as the rendering engine. Plain headless mode is detectable; the stealth layer patches the most common fingerprints before any page interaction. See ADR 003 for the full decision rationale.
...
See dependencies.md for rationale behind each dependency choice.
